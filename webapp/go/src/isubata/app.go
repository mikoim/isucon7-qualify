...
func addMessage(channelID, userID int64, content string) (int64, error) {
	res, err := db.Exec(
		"INSERT INTO message (channel_id, user_id, content, created_at) VALUES (?, ?, ?, NOW())", channelID, userID, content)
	if err != nil {
		return 0, err
	}
...
func queryMessages(chanID, lastID int64) ([]MessageWithUser, error) {
	...
	FROM
	    message AS m
		LEFT JOIN
	    user AS u ON m.user_id = u.id
	WHERE
	    m.id > ? AND m.channel_id = ?
	ORDER BY id DESC
	LIMIT 100`
	msgs := []MessageWithUser{}
	err := db.Select(&msgs, query, lastID, chanID)
	return msgs, err
}
...
func queryHaveReadsByChannel(userID int64) ([]HaveRead, error) {
	query := ` SELECT 
	    id as channel_id, IFNULL(message_id, -1) as message_id
	FROM
	    channel AS c
        LEFT JOIN
	    (SELECT 
		...
		haveread
	    WHERE
        user_id = ?) as h ON c.id = h.channel_id
	`
	res := []HaveRead{}
	err := db.Select(&res, query, userID)
	return res, err
}
...
func fetchUnread(c echo.Context) error {
	...
	haveReads, err := queryHaveReadsByChannel(userID)
	if err != nil {
		return err
	}
...
func getHistory(c echo.Context) error {
	...
	//	r, err := jsonifyMessage(messages[i])
		...
		r := make(map[string]interface{})
		r["id"] = messages[i].ID
		r["user"] = User{Name: messages[i].Name, DisplayName: messages[i].DisplayName, AvatarIcon: messages[i].AvatarIcon}
